...
return_type lora_aes_set_key( const uint8_t key[], length_type keylen, aes_context ctx[1] )
{
    uint8_t cc, rc, hi;
...
return_type laes_encrypt( const uint8_t in[N_BLOCK], uint8_t  out[N_BLOCK], const aes_context ctx[1] )
{
    if ( ctx->rnd ) {
                             ...
                             int32_t n_block, uint8_t iv[N_BLOCK], const aes_context ctx[1] )
{
    ...
    while (n_block--) {
        xor_block(iv, in);
        if (laes_encrypt(iv, iv, ctx) != EXIT_SUCCESS) {
            return EXIT_FAILURE;
        }
