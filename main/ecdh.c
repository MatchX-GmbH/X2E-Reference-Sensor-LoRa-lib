  ...
  */
       ...
       && !bitvec_is_zero(r))
  {
    ...
    if (!success)
    {
      ...
      for (i = 0; i < BITVEC_NWORDS; ++i)
      {
        WARN_PRINTF("%.08x", (unsigned int)x1[i]);
      }
      WARN_PRINTF("' [%u]\n", (unsigned int)i);
      WARN_PRINTF("r = '");
      for (i = 0; i < BITVEC_NWORDS; ++i)
      {
        WARN_PRINTF("%.08x", (unsigned int)r[i]);
      }
      WARN_PRINTF("' [%u]\n", (unsigned int)i);
    }
  }
